...
def filter_mona(spectra):
    return [block for block in spectra if "fragmentation_method" in block and block["fragmentation_method"].lower() in ["hcd", "cid"] and "collision_energy" in block and str(block["collision_energy"]) in ["[45.0]", "[35.0]", "[65.0]"]]

def filter_gnps_library(spectra):
    instrument_regex = re.compile(".*(orbitrap|q exactive|q-exactive|qexactive|exploris).*", re.IGNORECASE)
    return [block for block in spectra if "name" in block and "instrument" in block and instrument_regex.search(block["instrument"]) and "collision_energy" in block and 10 <= float(block["collision_energy"].replace("[", "").replace("]", "")) <= 90]

data_folderer_path = "../../data"
input_data = {
    #"BOKU_iBAM_MB": {"mgf_file": f"{data_folderer_path}/BOKU_iBAM.mgf", "smiles_field": "smiles", "name_field": "name", "sf_field": "formula", "filter": filter_boku},
...
for database_name in input_data.keys():
    ...
    found_results, spectra = process_database(database_name, mgf_file, smiles_field, name_field, sf_field, checks, f"{pathlib.Path(mgf_file).parent}/derived/", filter_fn=filter_fn, verbose=include_details)
